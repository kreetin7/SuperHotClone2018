...
public class cubeProtoMiove : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Input.GetKey(KeyCode.W)) //if W is pressed 
		{
			transform.Translate(0f, 0f,
				1f * CubeSpeed); //moving the cube forwards on the Z axis because thats the orientation of Ryan's scene for some reason
		...
		if (Input.GetKey(KeyCode.S)) //if S is pressed 
		{
			...
			TimeManager.instance.TimeTarget =
				1; //setting the target to the normal rate so that it speeds back up to a "normal" speed
			//Time.timeScale = 1;
	...
	//JACKS STUFF BELOW FOR GAME MANAGER
